...
- Phase 4 — Email/Phone Verification (email change verification, phone verification): **COMPLETE** — committed (`52e4813`), pushed, GitHub Actions GREEN (backend + frontend, all steps), Vercel production deployment SUCCESS for both `commandcenter` and `commandcenter-backend` at this exact commit. India DLT/MSG91 production credential setup remains a separate, non-blocking operational track (code-complete, not yet configured for real SMS delivery — see below) — architecture/security audit complete (`PROFILE_PHASE4_EMAIL_PHONE_VERIFICATION_AUDIT.md`); Step 1 implemented (database migration for the 9 approved columns, mirrored into `schema.sql`, and `GET /api/users/me` now exposes `is_verified`). **Email-change backend: COMPLETE.** `POST /api/users/me/request-email-change`, `POST /api/users/me/resend-email-change-verification`, `POST /api/auth/verify-email-change` — current-password confirmation, hashed one-time token (1-hour expiry, superseded on resend/re-request), verification sent to the new address only, in-app security notifications (new `email_change` preference group) at request and completion, per-user rate limiting (3/hour request, 5/hour resend) and an IP-only limiter on verification, `email_changed_at` added to JWT session-invalidation (new `ecv` claim in `jwt.ts`, mirroring `password_changed_at`'s `pwv`) with all refresh tokens revoked atomically on successful verification. `is_verified` is untouched by this flow (by design — it remains signup-verification only). `GET /api/users/me` additionally now returns `pending_email` (caller's own, read-only) so the frontend's pending-change banner survives a page reload. 33 backend tests in `backend/tests/emailChange.test.ts`, all passing. **Email-change frontend/UI: COMPLETE.** `Profile.tsx`'s email field replaced the old "coming in a future phase" placeholder with a live verified badge (from `is_verified`), an inline "Change email" form (current email + password, matching the page's own existing inline-toggle convention for password change rather than a new modal system), a pending-change banner (current + pending address, resend action, no cancel — the backend has none), and full request/resend/error handling. New unauthenticated route `/verify-email-change` (`VerifyEmailChange.tsx`, mirroring `ResetPassword.tsx`'s no-auto-login pattern exactly, since `verify-email-change` never returns a session) reads the token from the URL, submits it exactly once, clears local auth state via `logout()` on success, strips the token from the visible URL, and never persists/logs the raw token. 26 new frontend tests (`Profile.test.tsx` additions + new `VerifyEmailChange.test.tsx`). **Phone verification backend: COMPLETE.** Full audit: `PROFILE_PHASE4_PHONE_SMS_PROVIDER_AUDIT.md`. Vendor: **MSG91**, OTP ownership: **CommandCenter-managed** (see audit for rationale). `POST /api/users/me/request-phone-verification`, `POST /api/users/me/resend-phone-verification`, `POST /api/users/me/verify-phone` — E.164 normalization (new `libphonenumber-js` dependency), 6-digit `crypto.randomInt` OTP hashed at rest, 10-minute expiry, 5-attempt ceiling (invalidates on the 5th failure), 60-second resend cooldown (derived from `phone_otp_expires`, no new column), per-user 5/hour rate limiting on all three endpoints mounted after `authenticate`. `SmsProvider` abstraction built (`ConsoleSmsProvider` default — zero real MSG91 credential needed in tests/CI/dev; `Msg91SmsProvider` for production, plain `fetch`, no new SDK). Phone writes fully isolated from the generic profile-update allowlist (a client can never set `phone_number`/`phone_verified` via `PUT /me/profile`). `GET /api/users/me` now also returns the caller's own `phone_number`/`phone_verified`; confirmed still absent from the teammate-scoped `GET /api/users`. New `phone_verification` notification preference, in-app notice on success. Phone remains explicitly NOT usable for login/recovery/2FA. 33 backend tests in `backend/tests/phoneVerification.test.ts`, all passing; full backend suite 597/597, 42/42 suites. **Phone verification frontend/UI: COMPLETE.** `Profile.tsx` gained a "Phone" section (own heading, matching every other Profile subsection) with four states — no-phone (input + "Send code"), OTP-pending (masked number, 6-digit input, "Verify"/"Resend code"), verified (masked number + "Verified" badge + "Change number"), change-number (reuses the entry form, "Cancel" returns to verified). A new `maskPhoneNumber()` helper reveals only the last 4 digits, country-code-length-agnostic (not India-hardcoded). No "Remove" action (no backend endpoint exists for it). OTP input strips non-digits and is never persisted in browser storage; every action button disables itself while its own request is in flight. `services/api.ts` gained three thin wrappers (`requestPhoneVerification`/`resendPhoneVerification`/`verifyPhone`). 37 new frontend tests. **State-consistency fix: COMPLETE.** The reload-mid-change edge case discovered during the frontend task was root-caused and fixed properly rather than left as a documented limitation: `setPendingPhoneOtp` now also sets `phone_verified = false` in the same UPDATE that sets the new `phone_number`, so an already-verified account requesting a different number is correctly unverified immediately, in the database, not just in optimistic client state — a page reload mid-change now reports the correct pending state. `resendPhoneVerification`'s guard was extended to reject resend against an already-verified number, closing a gap the fix itself would otherwise open. `requestPhoneVerification`'s response now includes an authoritative `phone_verified` field; the frontend reads it directly instead of hardcoding a value. 3 new backend regression tests (A-H lifecycle: verify → request a different number → confirm unverified via both the response and a live `GET /me` → verify → confirm verified again) plus 1 new frontend test, all passing. India DLT registration/MSG91 production account setup (operational, not code) remains not started — production SMS delivery is not claimed to work yet, but this no longer blocks Phase 4's own code-completeness, which is now genuinely ready for final closeout.
