...
import { ALLOWED_PRESET_IDS } from './avatars.presets';
...
export const avatarService = {
  ...
  /**
   * Set user's avatar to a trusted application preset.
   */
  async setPresetAvatar(
    userId: string,
    presetId: string,
  ): Promise<{ avatar_key: string; avatar_url: string }> {
    if (!ALLOWED_PRESET_IDS.includes(presetId)) {
      throw new BadRequestError('Invalid preset avatar ID');
    }

    const presetKey = `preset:${presetId}`;

    // Clean up old non-preset avatar if exists
    const oldResults = await query('SELECT avatar_key FROM users WHERE user_id = $1', [userId]);
    const oldAvatarKey = oldResults[0]?.avatar_key;
    ...
    await query(
      `UPDATE users
       SET avatar_key = $1,
           ...
           avatar_width = 256,
           avatar_height = 256,
           avatar_uploaded_at = $2,
           updated_at = $2
       WHERE user_id = $3`,
      [presetKey, now, userId],
    );

    if (oldAvatarKey && !oldAvatarKey.startsWith('preset:')) {
      avatarStorageService.deleteAvatar(oldAvatarKey).catch((err) => {
        console.warn('[avatars.service] Failed to delete old avatar:', oldAvatarKey, err);
      });
    }

    return {
      avatar_key: presetKey,
      avatar_url: avatarStorageService.getAvatarUrl(presetKey),
    };
  },
