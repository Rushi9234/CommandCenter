...
export const setPresetAvatar = async (req: AuthRequest, res: Response) => {
  const { preset_id } = req.body;
  if (!preset_id || typeof preset_id !== 'string') {
    throw new BadRequestError('Preset avatar ID is required');
  }

  const userId = req.user!.userId;
  const result = await avatarService.setPresetAvatar(userId, preset_id);
