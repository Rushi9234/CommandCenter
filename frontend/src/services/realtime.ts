export interface RealtimeEvent {
  ...
  // Chat V1 Task 4/6 -- an identifier only, never message content (see
  // the backend's realtime.types.ts for the full rationale). Optional
  // like every other field here since only chat.message_created sets it.
...
export class RealtimeClient {
  ...
  start() {
    ...
    if (!this.stopped && this.activeToken === token) return;
    this.stop();
    this.stopped = false;
    void this.connect();
  }

  ensureToken(token: string | null) {
